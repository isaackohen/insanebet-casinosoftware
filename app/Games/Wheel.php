...
class Wheel extends QuickGame implements MultiplierCanBeLimited
{
    ...
    public function start($user, Data $data)
    {
        if ($data->game()->mode === 'double') {
            return new SuccessfulQuickGameResult((new ProvablyFair($this))->result(), function (SuccessfulQuickGameResult $result) use ($user, $data) {
                ...
                $target = null;
                if ($data->game()->target === 'green') {
                    $target = $green;
                } elseif ($data->game()->target === 'red') {
                    $target = $red;
                } elseif ($data->game()->target === 'black') {
                    $target = $black;
                }

                if ($this->isLoss($result->provablyFairResult(), $data)) {
                    $result->lose();
                } else {
                    $result->win($user, $data, $target == $red || $target == $black ? HouseEdgeModule::apply($this, 2) : HouseEdgeModule::apply($this, 14));
                }

                if (in_array($result->provablyFairResult()->result()[0], $green)) {
                    $color = 'green';
                } elseif (in_array($result->provablyFairResult()->result()[0], $red)) {
                    $color = 'red';
                } elseif (in_array($result->provablyFairResult()->result()[0], $black)) {
                    $color = 'black';
                }
            ...
            return new SuccessfulQuickGameResult((new ProvablyFair($this, $this->client_seed()))->result(), function (SuccessfulQuickGameResult $result) use ($user, $data) {
                ...
                if ($data->game()->target === 'green') {
                    ...
                    $target = $black;
                } elseif ($data->game()->target === 'yellow') {
                    $target = $yellow;
                }
                ...
                if ($this->isLoss($result->provablyFairResult(), $data)) {
                    ...
                    $result->win($user, $data, $target == $yellow ? HouseEdgeModule::apply($this, 50) : ($target == $red ? HouseEdgeModule::apply($this, 3) : ($target == $green ? HouseEdgeModule::apply($this, 5) : HouseEdgeModule::apply($this, 2))));
                }

                if (in_array($result->provablyFairResult()->result()[1], $yellow)) {
                    $color = 'yellow';
                } elseif (in_array($result->provablyFairResult()->result()[1], $green)) {
                    $color = 'green';
                } elseif (in_array($result->provablyFairResult()->result()[1], $red)) {
                    $color = 'red';
                } elseif (in_array($result->provablyFairResult()->result()[1], $black)) {
                    $color = 'black';
                }
            ...
            return new RejectedGameResult(1, 'Invalid game mode');
        }
    }
    ...
    public function multiplier(?\App\Game $game, ?Data $data, ProvablyFairResult $result): float
    {
        ...
        if ($target === 'x50') {
            return $data->game()->target === 'yellow' ? HouseEdgeModule::apply($this, 50) : ($data->game()->target === 'red' ? HouseEdgeModule::apply($this, 3) : ($data->game()->target === 'green' ? HouseEdgeModule::apply($this, 5) : HouseEdgeModule::apply($this, 2)));
        } else {
            return $data->game()->target === 'red' || $data->game()->target === 'black' ? HouseEdgeModule::apply($this, 2) : HouseEdgeModule::apply($this, 14);
        }
    }
