...
class Mines extends ExtendedGame implements MultiplierCanBeLimited
{
    ...
    public function turn(Game $game, array $turnData): Turn
    {
        if (is_array($turnData['id'])) {
            foreach ($turnData['id'] as $id) {
                ...
                if (! ($state instanceof ContinueGame)) {
                    return $state;
                }
        ...
        if ($this->inHistory($game, $turnData['id'])) {
            return new FailedTurn($game, []);
        }
        $this->pushHistory($game, $turnData['id']);

        if ($this->getGrid($game, $game->server_seed)[intval($turnData['id'])] == 1) {
            return new LoseGame($game, ['grid' => $this->getGrid($game, $game->server_seed)]);
        }
        ...
        if ($this->getTurn($game) - 1 == (5 * 5) - 1 - intval($this->userData($game)['data']['mines'])) {
            return new FinishGame($game, $this->getGrid($game, $game->server_seed));
        }
    ...
    public function isLoss(ProvablyFairResult $result, Game $game, array $turnData): bool
    {
        if ($this->getTurn($game) > 1) {
            for ($i = 1; $i < $this->getTurn($game); $i++) {
                if ($this->getGrid($game, $result->server_seed())[$this->gameData($game)[strval($i)]] != 0) {
                    return false;
                }
            }
        }
    ...
    public function result(ProvablyFairResult $result): array
    {
        ...
        for ($bombs = 2; $bombs <= 24; $bombs++) {
            ...
            $tiles = [];
            for ($i = 0; $i < $bombs; $i++) {
                array_push($tiles, array_splice($squares, floor($result->extractFloats($bombs)[$i] * ($max_squares - $i + 1)), 1)[0]);
            }
