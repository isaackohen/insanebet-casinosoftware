...
class Dice extends QuickGame implements MultiplierCanBeLimited
{
    ...
    {
        if ($data->game()->target !== 'lower' && $data->game()->target !== 'higher') {
            return new RejectedGameResult(1, 'Invalid dice target');
        }
        if ($data->game()->target == 'lower' && $data->game()->value > 110) {
            return new RejectedGameResult(2, 'Invalid value');
        }
        if ($data->game()->target == 'higher' && $data->game()->value < 2) {
            return new RejectedGameResult(2, 'Invalid value');
        }
        if ($data->game()->value < 10 || $data->game()->value > 110) {
            return new RejectedGameResult(2, 'Invalid value');
        }
        ...
        return new SuccessfulQuickGameResult((new ProvablyFair($this))->result(), function (SuccessfulQuickGameResult $result, array $transformedResults) use ($user, $data) {
            ...
            if ($this->isLoss($result->provablyFairResult(), $data)) {
                ...
                $result->win($user, $data, $this->payout($low - 10, $high - 10));
            }
