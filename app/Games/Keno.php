...
class Keno extends QuickGame implements MultiplierCanBeLimited
{
    ...
    {
        if (! isset($data->game()->tiles) || count($data->game()->tiles) < 1 || count($data->game()->tiles) > 10) {
        ...
        return new SuccessfulQuickGameResult((new ProvablyFair($this))->result(), function (SuccessfulQuickGameResult $result, array $transformedResults) use ($user, $data) {
            ...
            foreach ($transformedResults as $tile) {
                if (in_array($tile, $data->intArray('tiles'))) {
                    $hits++;
                }
            }
    ...
    public function result(ProvablyFairResult $result): array
    {
        ...
        $tiles = [];
        for ($i = 0; $i < 10; $i++) {
            array_push($tiles, array_splice($squares, floor($result->extractFloats(10)[$i] * ($max_squares - $i + 1)), 1)[0]);
        }
