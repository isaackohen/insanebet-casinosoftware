...
abstract class BitcoinCurrency extends Currency
{
    public function process(string $wallet = null)
    {
        ...
        if (isset($contents['details'][0]['address'])) {
            $this->accept($contents['confirmations'], $contents['details'][0]['address'], $contents['hex'], abs($contents['details'][0]['amount']));
        } elseif (isset($contents['details']['address'])) {
            $this->accept($contents['confirmations'], $contents['details']['address'], $contents['hex'], abs($contents['details']['amount']));
        }
    }
    ...
    public function newWalletAddress($accountName = null): string
    {
        try {
            ...
            if ($contents->error != null) {
                throw new \Exception('Exception during getnewaddress');
            }
    ...
    public function setupWallet(): ?string
    {
        try {
            ...
            if ($depositAccount === 'Error' || $withdrawAccount === 'Error') {
                return null;
            }
