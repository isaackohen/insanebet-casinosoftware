                ...
                @else
                    @foreach(\App\Withdraw::where('status', 0)->latest()->take(5)->get() as $withdraw)
                        ...
                        <div class="d-flex border-top pt-3">
                            <img src="{{ $user->avatar }}" class="avatar rounded me-3" alt="shreyu">
                            <div>
                                <h6 class="mt-1 mb-0 font-size-15">{{ $user->name }}</h6>
                                <h6 class="text-muted font-weight-normal mt-1 me-3">{{ number_format($withdraw->sum, 8, '.', ' ') }} {{ \App\Currency\Currency::find($withdraw->currency)->name() }}</h6>
                            </div>
                        </div>
                    @endforeach
                @endif
