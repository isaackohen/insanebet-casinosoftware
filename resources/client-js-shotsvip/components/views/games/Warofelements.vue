<template>
    <div>
        <div class="warofelements-game-field">
                  <video-js id="stream" class="vjs-default-skin" muted autoplay preload="auto">
                    <source src="https://static2.casinotv.com\/hls-live\/battle_smooth.m3u8" type="application/x-mpegURL">
                  </video-js>


            <div class="timer"></div>
        </div>
        <div class="warofelements-field">
            <div class="header">
            ...
            <div class="bets">
                <div class="content">
                    <div class="cell" data-chip="player">
                        <div class="title">{{ $t('general.warofelements.player') }}</div>
                ...
                <div class="content rows">
                    <div class="content">
                        <div class="cell" data-chip="draw">
                            <div class="title">{{ $t('general.warofelements.tie') }}</div>
                            <div class="payout">x6.00</div>
                        </div>
                    </div>
                    <!--
                    <div class="content" style="margin-top: 1px">
                        <div class="cell" data-chip="pair_player">
                            <div class="title">{{ $t('general.warofelements.pair_player') }}</div>
                            <div class="payout">6.00</div>
                        </div>
                        <div class="cell" data-chip="pair_banker">
                            <div class="title">{{ $t('general.warofelements.pair_banker') }}</div>
                            <div class="payout">6.00</div>
                        </div>
                    </div>
                    !-->
                ...
                <div class="content">
                    <div class="cell" style="margin-right: 0" data-chip="banker">
                        <div class="title">{{ $t('general.warofelements.banker') }}</div>
                        <div class="payout">x1.95</div>
                    </div>
                </div>
            </div>
        </div>
        <div class="warofelements-players">
            <overlay-scrollbars :options="{ scrollbars: { autoHide: 'leave' }, className: 'os-theme-thin-light' }" class="warofelements-players-scrollable os-host-flexbox">
                <div class="warofelements-players-container player">
                    <div class="header">{{ $t('general.warofelements.pair') }}</div>
                    <overlay-scrollbars :options="{ scrollbars: { autoHide: 'leave' }, className: 'os-theme-thin-light' }" class="users" data-container="pair_player">
                        <div class="empty">{{ $t('general.warofelements.empty') }}</div>
                    </overlay-scrollbars>
                </div>
                <div class="warofelements-players-container player">
                    <div class="header">{{ $t('general.warofelements.player') }}</div>
                    <overlay-scrollbars class="users" data-container="player" :options="{ scrollbars: { autoHide: 'leave' }, className: 'os-theme-thin-light' }">
                        <div class="empty">{{ $t('general.warofelements.empty') }}</div>
                    </overlay-scrollbars>
                </div>
                <div class="warofelements-players-container tie">
                    <div class="header">{{ $t('general.warofelements.tie') }}</div>
                    <overlay-scrollbars :options="{ scrollbars: { autoHide: 'leave' }, className: 'os-theme-thin-light' }" class="users" data-container="draw">
                        <div class="empty">{{ $t('general.warofelements.empty') }}</div>
                    </overlay-scrollbars>
                </div>
                <div class="warofelements-players-container banker">
                    <div class="header">{{ $t('general.warofelements.banker') }}</div>
...
<script>
        $(function() {
            var player = videojs('stream', {
              width: $('.game-content-warofelements').width(),
              height: ($('.game-content-warofelements').height() / 100) * 80,
              html5: {
                vhs: {
                  bandwidth: 4194304, // 8 * 1024 * 512 ~ 2Mbps
            ...
            player.play();
            console.log('Load videojs-http-streaming-module was performed.');
        })
        ...
        methods: {
            gameDataRetrieved() {
                if(this.gameInstance.game.data.timestamp > 0) {
                    ...
                    if (left > 0 && left <= 15) this.countdown($('.game-warofelements').find('.timer'), {
                        timePassed: left
                    });
            ...
            show(data) {
                ...
                $('.warofelementsCardsPlayer .score, .warofelementsCardsBanker .score').stop().removeClass('win').removeClass('draw').fadeOut(300);
                $('.warofelementsCardsPlayer .card, .warofelementsCardsBanker .card').stop().fadeOut(300, function() {
                    $(this).remove();
                });
                ...
                setTimeout(() => {
                    ...
                    this.chain(data.player.length, 500, (i) => {
                        ...
                        if(i === data.player.length) {
                            setTimeout(() => {
                                if(!v()) return;
                                $('.warofelementsCardsPlayer .score').html(data.score.player).fadeIn('fast');
                                ...
                                this.chain(data.dealer.length, 500, (i) => {
                                    ...
                                    if (i === data.dealer.length) {
                                        ...
                                        setTimeout(() => {
                                            if(!v()) return;
                                            $('.warofelementsCardsBanker .score').html(data.score.dealer).stop().fadeIn('fast');

                                            $('.warofelementsCardsPlayer .card, .warofelementsCardsPlayer .score').toggleClass('win', data.status === 'player');
                                            $('.warofelementsCardsBanker .card, .warofelementsCardsBanker .score').toggleClass('win', data.status === 'banker');
                                            $('.warofelementsCardsPlayer .card, .warofelementsCardsPlayer .score').toggleClass('draw', data.status === 'draw');
                                            $('.warofelementsCardsBanker .card, .warofelementsCardsBanker .score').toggleClass('draw', data.status === 'draw');
            ...
            sendCard(e, handIndex) {
                ...
                $(e.element).hide();
                $(`.warofelementsCards${e.hand}`).append(e.element);
            ...
            multiplayerEvent(event, data) {
                switch (event) {
                    ...
                    case 'MultiplayerTimerStart':
                        this.countdown($('.game-warofelements').find('.timer'), {
                            timePassed: 0
                        });
...
<style lang="scss">
    ...
    .game-warofelements {
        .game-content-warofelements {
            .fa-warofelements-ribbon {
                position: relative;
                left: 50%;
                top: 50%;
            ...
            @include themed() {
                .warofelements-players {
                    ...
                    .warofelements-players-container .os-host {
                        width: 100% !important;
                        flex: unset !important;
                    }

                    .warofelements-players-scrollable {
                        width: 100%;
                    ...
                    .warofelements-players-container {
                        display: flex;
                        flex-direction: column;
                        width: 100%;
                        min-width: 247px;
            ...
            .warofelementsCardsPlayer, .warofelementsCardsBanker {
                position: absolute;
                margin-top: 90px;
            ...
            .warofelementsCardsPlayer {
                left: 100px;
            }

            .warofelementsCardsBanker {
                right: 150px;
            }
