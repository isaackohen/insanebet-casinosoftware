        ...
        <div class="content">
            ...
            <div class="row">
                <div class="col-6">
                    ...
                    <span>{{ this.usd ? ('$' + wager.toFixed(2)) :  (wager.toFixed(currency.startsWith('local_') ? 2 : 8)) }}</span> <icon :icon="currencies[currency].icon" :style="{ color: currencies[currency].style }"></icon>
                </div>
                <div class="col-6">
                    <div>{{ $t('general.profit_monitoring.profit') }}</div>
                    <span>{{ this.usd ? ('$' + profit.toFixed(2)) : (profit.toFixed(this.currency.startsWith('local_') ? 2 : 8)) }}</span> <icon :icon="currencies[currency].icon" :style="{ color: currencies[currency].style }"></icon>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters(['currencies', 'currency', 'gameInstance', 'isGuest', 'user', 'theme', 'usd'])
        },
        methods: {
            pushStats(game) {
                ...
                if(game.win || game.status === 'win') {
                    this.wins += 1;
                    this.profit += (game.profit * this.currencies[this.currency].price) - this.gameInstance.bet;
                } else {
                    this.losses += 1;
            ...
            pushToSeries() {
                ...
                this.series.push({
                    ...
                    y: this.usd ? parseFloat(this.profit.toFixed(2)) : parseFloat(this.profit.toFixed(this.currency.startsWith('local_') ? 2 : 8))
                });
            },
            reset() {
                this.wager = 0;
